...
const Card = ({ id, title, description, animatedShape, delay = 0 }) => {
  const icons = {
    masteryhub: (
      <svg className="w-6 h-6 md:w-8 md:h-8 mr-2 md:mr-3 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <section id={id} className={`w-full max-w-[1440px] mx-auto px-4 ${id === 'masteryhub' ? 'mt-8 md:mt-[100px]' : 'mt-8'}`}>
      <div 
        className="w-full max-w-[1100px] mx-auto flex flex-col md:flex-row card-bg rounded-lg overflow-hidden card-trigger"
        style={{ animationDelay: `${delay}s` }}
      >
        <div className="w-full md:w-1/2 p-6 md:p-12 flex flex-col justify-start">
          <div className="flex items-center mb-4 md:mb-6">
            {icons[id]}
            <h2 className="text-primary text-2xl md:text-3xl">
              {title}
            </h2>
          </div>
          <div className="overflow-y-auto flex-grow pr-2 md:pr-4" style={{ maxHeight: "300px" }}>
            <p className="text-current text-base md:text-lg mb-4 md:mb-8">
              {description}
            </p>
          </div>
          <a 
            href={getPageUrl(id)}
            className="text-action hover:text-primary transition-colors text-base md:text-lg"
          >
